...
class StructureFlow():
    def __init__(self, config):
        ...
        self.checkpoints_path = os.path.join(
            config.PATH, config.NAME, 'checkpoints')
        self.test_image_path = os.path.join(
            config.PATH, config.NAME, 'test_result')
    ...
    def train(self):
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                # structure model
                if model == 1:
                    logs = self.flow_model.update_structure(
                        inputs, smooths, maps, landmark)
                    iterations = self.flow_model.iterations
                # flow model
                elif model == 2:
                    logs = self.flow_model.update_flow(
                        inputs, smooths, gts, maps, self.flow_model.use_correction_loss, self.flow_model.use_vgg_loss)
                    iterations = self.flow_model.iterations
                # flow with structure model
                elif model == 3:
                    logs = self.flow_model.update_model(
                        inputs, smooths, gts, maps, landmark)
                    iterations = self.flow_model.iterations
                ...
                if self.config.SAMPLE_INTERVAL and iterations % self.config.SAMPLE_INTERVAL == 0:
                    ...
                    result, flow = self.flow_model.sample(
                        inputs, smooths, gts, maps)
    ...
    def eval(self, writer=None):
        val_dataset = Dataset(
            self.config.DATA_VAL_GT, self.config.DATA_VAL_STRUCTURE, self.config, self.config.DATA_VAL_MASK)
        ...
        if writer is not None:
            ...
            writer.add_scalar('eval_ssim', avg_ssim, iterations)
            writer.add_scalar('eval_l1', avg_l1, iterations)
    ...
    def test(self):
        ...
        test_dataset = Dataset(
            self.config.DATA_TEST_GT, self.config.DATA_TEST_STRUCTURE, self.config, self.config.DATA_TEST_MASK)
            ...
            for items in test_loader:
                ...
                for i in range(outputs_merged.size(0)):
                    ...
                    if self.debug and model == 3:
                        smooth_ = self.postprocess(
                            smooth_stage_1[i, :, :, :].unsqueeze(0))*255.0
                        inputs_ = self.postprocess(
                            inputs_show[i, :, :, :].unsqueeze(0))*255.0
                        gts_ = self.postprocess(
                            gts[i, :, :, :].unsqueeze(0))*255.0
                        print(path)
    ...
    def write_image(self, result, train_writer, iterations, label):
        if result:
            name = '%s/model%d_sample_%08d' % (self.samples_path,
                                               self.config.MODEL, iterations) + label + '.jpg'
            write_2images(result, self.config.SAMPLE_SIZE, name)
            write_2tensorboard(iterations, result, train_writer,
                               self.config.SAMPLE_SIZE, label)
    ...
    def metrics(self, inputs, gts):
        ...
        for i in range(inputs.size(0)):
            inputs_p = inputs[i, :, :, :].cpu().numpy().astype(
                np.float32).transpose(1, 2, 0)
            gts_p = gts[i, :, :, :].cpu().numpy().astype(
                np.float32).transpose(1, 2, 0)
            psnr_value.append(psnr(inputs_p, gts_p, data_range=1))

        psnr_value = np.average(psnr_value)
        inputs = inputs.view(
            b*n, w, h).cpu().numpy().astype(np.float32).transpose(1, 2, 0)
        gts = gts.view(
            b*n, w, h).cpu().numpy().astype(np.float32).transpose(1, 2, 0)
        ssim_value = ssim(inputs, gts, data_range=1,
