...
class StructureFlowModel(BaseModel):
    ...
    def update_structure(self, inputs, smooths, maps, landmarks):
        ...
        # 논문에서는 reconstruction된 이미지와 그 이미지를 downscaling한 이미지에 대해서 2개의 discriminator를 씀
        # 따라서 fakelabel이 2개여서 아래와 같은 코딩인듯
        for i in range(len(fake_labels)):
...
############################################################ fix #####################################################################
        mask_weight = gaussian_filter(maps.cpu(), sigma=self.config.STRUCTURE_SIGMA)
        ...
        self.landmark_loss = torch.norm((landmarks-output_land).reshape(-1,self.s_land.point_num*2),2,dim=1,keepdim=False).sum() * self.config.STRUCTURE_LANDMARK
############################################################ fix #####################################################################
